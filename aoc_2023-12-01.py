...
DIGIT_MAP  = dict(zip(DIGITS.split("|"),range(1,10)))
DIGIT = re.compile(f"\d|{DIGITS}")

to_int = lambda x : DIGIT_MAP.get(x, x)
def two_digits(str):
    all_digits = []
    start = 0
    while True:
        x = re.search(DIGIT, str[start:])
        if x is not None:
            all_digits.append(x.group(0))
            start += x.start()+1
        else:
            break
    res = int(
        f"{to_int(all_digits[0])}{to_int(all_digits[-1])}"
    )
    return res
...
print( 
    sum(
        two_digits(x) for x in aoc_1_data.split("\n")[1:-1]
    )
)
